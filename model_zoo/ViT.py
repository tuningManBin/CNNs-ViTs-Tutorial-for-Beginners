...
class ParchEmbedding(nn.Module):
    ...
    """
    def __init__(self, in_channels: int, image_size: int, patch_size: int, embedding_dim: int, dropout_ratio: float = 0.1):
        """
        Arguments:
            in_channels (int): the number of input tensor channels
            image_size (int): the size of images (h = w)
            patch_size (int): the size of pixel patches (h = w)
            embedding_dim (int): the dimensions of an embedding
...
class SelfAttention(nn.Module):
    ...
    """
    def __init__(self, embedding_dim: int, num_heads: int, attn_dropout_ratio: float = 0.1, proj_dropout_ratio: float = 0.1):
        """
        Arguments:
            embedding_dim (int): the dimensions of an embedding
            num_heads (int): the number of self-attention heads
            attn_dropout_ratio (float): the probability of an element being zero in attention map
            proj_dropout_ratio (float): the probability of an element being zero in projection
        """
...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: Optional[int] = None, output_dim: Optional[int] = None,
                 activation: Type[nn.Module] = nn.GELU, dropout_ratio: float = 0.1):
        """
        Arguments:
            input_dim (int): the dimensions of input tensor
            hidden_dim (Optional[int]): the dimensions of output tensor in hidden layer
            output_dim (Optional[int]): the dimensions of output tensor
            activation (Type[nn.Module]): activation function
            dropout_ratio (float): the probability of an element being zero
        """
...
class BasicBlock(nn.Module):
    ...
    """
    def __init__(self, embedding_dim: int, num_heads: int, mlp_ratio: int):
        """
        Arguments:
            embedding_dim (int): the dimensions of an embedding
            num_heads (int): the number of self-attention heads
...
class VisionTransformer(nn.Module):
    ...
    """
    def __init__(self, image_channels: int, image_size: int, pool_stride: int,
                 patch_size: int, embedding_dim: int, num_heads: int, depth: int, mlp_ratio: int,
                 dataset: str, normalize: bool, logits_dim: int):
        """
        Arguments:
    ...
    def forward_loss(self, logits: torch.Tensor, ground_truth: torch.Tensor):
        """
        This function is used to calculate loss.
        """
        loss = nn.CrossEntropyLoss(reduction="mean")(logits, ground_truth)
        return loss
